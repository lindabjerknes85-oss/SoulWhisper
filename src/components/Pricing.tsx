...
export function Pricing({ onClose }: PricingProps) {
  ...
  const loadPricingData = async () => {
    console.log('🔥 Pricing.tsx loadPricingData() CALLED at', new Date().toISOString());
    try {
      console.log('🔥 Querying subscription_plans...');
      ...
      console.log('🔥 Raw plansData:', plansData);
      ...
      if (plansData && plansData.length > 0) {
        console.log('🔥 Processing', plansData.length, 'plans');

        plansData.forEach((plan, index) => {
          console.log(`🔥 Plan ${index + 1}:`, {
            name: plan.name,
            has_stripe_link: !!plan.stripe_payment_link,
            stripe_link_value: plan.stripe_payment_link,
            stripe_link_length: plan.stripe_payment_link?.length || 0,
            stripe_link_type: typeof plan.stripe_payment_link
          });
        });
  ...
  const handleSelectPlan = (plan: SubscriptionPlan) => {
    ...
    console.log('🚀 Payment link:', plan.stripe_payment_link);
    console.log('🚀 Payment link type:', typeof plan.stripe_payment_link);
    console.log('🚀 Payment link truthy?:', !!plan.stripe_payment_link);
    ...
    console.log('✅ Opening payment link:', plan.stripe_payment_link);
